/* BMW HMI icon set — line-art, 1.75–2px stroke, 24×24 viewBox.
   All icons are pure SVG strings; consume via:  <span dangerouslySetInnerHTML={{__html: BMWIcons.fan}} />
   Or use the <Icon name="fan" size={32} /> helper.

   Stroke style follows BMW iD Drive 8/9 — geometric, slightly rounded
   line caps, no fills (except indicator dots).
*/
window.BMWIcons = (() => {
  ...
  return {
    ...
    arrow:   wrap('<path d="M12 4l8 8h-5v8h-6v-8H4l8-8z"/>'),  // navigation maneuver arrow
    shield:  wrap('<path d="M12 3l8 3v6c0 5-3.5 8-8 9-4.5-1-8-4-8-9V6l8-3z"/>'),
  };
})();

window.BMWIcon = function ({ name, size = 24, color = "currentColor", style = {}, className = "" }) {
  const html = window.BMWIcons[name] || "";
  return React.createElement("span", {
    className: "bmw-icon " + className,
